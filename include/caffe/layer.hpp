...
class Layer {
 public:
  ...
  explicit Layer(const LayerParameter& param)
    : layer_param_(param) {
	  ...
	  if (param.has_int8_inference() == true) {
		  int8_inference_ = param.int8_inference();
	  }else {
  ...
  //INT8 Edited
  /*const float activation_scale_factor() const {
	  return (const float)activation_scale_factor_;
  }
  const vector<float> weight_scale_factor() const {
	  return (const vector<float>)weight_scale_factor_;
  }*/
  bool int8_inference() {
  ...
  //INT8 Edited: TESTING
  void Quantization_int8(const vector<Blob<Dtype>*>& bottom,
		  const vector<Blob<char>*>& bottom_int8);
  void Dequantization_int8(const vector<Blob<int>*>& top_int,
		  const vector<Blob<Dtype>*>& top);
  virtual void Forward_int8_cpu(const vector<Blob<char>*>& bottom,
		  const vector<Blob<int>*>& top) {
	  LOG(INFO)<<"int8 Kernel isn't defined";
  }
 ...
 protected:
  ...
  void Dequantization_int8(const vector<Blob<int>*>& top_int,
		  const vector<Blob<Dtype>*>& top);
  */ /*testing: to public*/
  void Quantization_int8_gpu(const vector<Blob<Dtype>*>& bottom,
		  const vector<Blob<char>*>& bottom_int8);
  void Dequantization_int8_gpu(const vector<Blob<int>*>& top_int,
		  const vector<Blob<Dtype>*>& top);
  ...
  virtual void Forward_int8_gpu(const vector<Blob<char>*>& bottom,
		  const vector<Blob<int>*>& top) {
	  return Forward_int8_cpu(bottom, top);
  }

  virtual void Forward_bias_int8_cpu(const vector<Blob<Dtype>*>& bottom,
		  const vector<Blob<Dtype>*>& top) {
	  LOG(INFO)<<"bias Kernel of int8 isn't defined";
  }

  virtual void Forward_bias_int8_gpu(const vector<Blob<Dtype>*>& bottom,
		  const vector<Blob<Dtype>*>& top) {
	  return Forward_bias_int8_cpu(bottom, top);
  }
