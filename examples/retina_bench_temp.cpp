...
int main(void)
{
	...
	FILE* caffe_result;
	caffe_result = fopen("caffe_result_th0.02_nms_0.20_int8.txt", "w");
			...
			for (int j = 0; j < cols_num; j++) {
					...
					* objects in input_channels. */
					...
					for (int u = 0; u < faceInfo.size(); u++) {
						...
						sprintf(score, "%f", faceInfo[u].score);
						fprintf(caffe_result, "%s %s %s %s %s %s\n", directory_parser[directory_parser.size()-1].c_str(), xmin, ymin, xmax, ymax, score);
					}
