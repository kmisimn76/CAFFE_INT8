...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    loss += layer_loss;
		...
		timersub(&e_t, &s_t, &timer);
//		printf("Layer Time: %lf\n", timer.tv_sec*1000.0+timer.tv_usec/1000.0);
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
	  ...
	  //INT8 Edited
	  if (j==0 && layers_[target_layer_id]->int8_inference()) { //int8 , weight only
		if (!target_int8_blobs[j]->ShapeEquals(source_layer.blobs(j))) {
			Blob<char> source_blob;
        	const bool kReshape = true;
        	source_blob.FromProto(source_layer.blobs(j), kReshape);
        	LOG(FATAL) << "Cannot copy param " << j << " weights from layer '"
        	    << source_layer_name << "'; shape mismatch.  Source param shape is "
		...
		target_int8_blobs[j]->FromProto(source_layer.blobs(j), kReshape);
		//std::cout<<"int8 weights: "<<source_layer_name<<": "<<target_int8_blobs.get()->count()<<"\n";
		//for(int kk=0;kk<target_int8_blobs.get()->count();kk++){
		//	std::cout<<(int)target_int8_blobs.get()->cpu_data()[kk]<<" ";
		//}
		DLOG(INFO) << "Copy Int8 Blob Layer " <<source_layer_name;
	  }
	  else { //not int8 OR bias
