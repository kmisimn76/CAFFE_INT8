...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
	...
	if (this->int8_inference_) {
		this->int8_blobs_[0].reset(new Blob<char>(weight_shape));
		/** int8 only for inference: No need to use filler */
		// Filler for Test
		shared_ptr<Filler<char> > weight_filler(GetFiller<char>(
					this->layer_param_.convolution_param().weight_filler()));
		weight_filler->Fill(this->int8_blobs_[0].get());
	}
	else {	
  ...
  //INT8 Edited
  if (this->int8_inference_) {
	  kernel_dim_ = this->int8_blobs_[0]->count(1);
  } else {
...
void BaseConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  col_buffer_int8_.Reshape(col_buffer_shape_);
...
void BaseConvolutionLayer<Dtype>::forward_cpu_gemm_int8_conv(const char* input,
  ...
  if (!is_1x1_) {
    if (!skip_im2col) {
      ...
      conv_im2col_cpu_int8_conv(input, col_buffer_int8_.mutable_cpu_data());
    }
