...
void caffe_conv(const Blob<char>* in, ConvolutionParameter* conv_param,
    const vector<shared_ptr<Blob<char> > >& weights,
    Blob<int>* out) {
  ...
  int* out_data = out->mutable_cpu_data();
                    ...
                    for (int q = 0; q < kernel_w; q++) {
                      ...
                      if (in_z >= 0 && in_z < (has_depth ? in->shape(2) : 1)
                        ...
                        out_data[out->offset(out_offset)] +=
                            (int)(in->data_at(in_offset))
                            * (int)(weights[0]->data_at(weight_offset));
                      }
                    }
                  }
                }
...
void caffe_conv_(const Blob<Dtype>* in, ConvolutionParameter* conv_param,
...
template void caffe_conv_(const Blob<float>* in,
    ConvolutionParameter* conv_param,
    const vector<shared_ptr<Blob<float> > >& weights,
    Blob<float>* out);
template void caffe_conv_(const Blob<double>* in,
    ConvolutionParameter* conv_param,
    const vector<shared_ptr<Blob<double> > >& weights,
    Blob<double>* out);

template <typename TypeParam>
class ConvolutionInt8E2ELayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ConvolutionInt8E2ELayerTest()
      : blob_bottom_(new Blob<Dtype>(2, 3, 100, 100)),
        blob_bottom_2_(new Blob<Dtype>(2, 3, 100, 100)),
  ...
  virtual ~ConvolutionInt8E2ELayerTest() {
    delete blob_bottom_;
...
/*void caffe_conv(const Blob<char>* in,
    ConvolutionParameter* conv_param,
    const vector<shared_ptr<Blob<char> > >& weights,
    Blob<int>* out);
*/
template <typename TypeParam>
class ConvolutionInt8LayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ConvolutionInt8LayerTest()
      : blob_bottom_(new Blob<Dtype>(2, 4, 32, 32)),
        blob_bottom_2_(new Blob<Dtype>(2, 4, 32, 32)),
        blob_top_(new Blob<Dtype>()),
        blob_top_2_(new Blob<Dtype>()) {}
  virtual void SetUp() {
    // fill the values
    FillerParameter filler_param;
    filler_param.set_value(1.);
	filler_param.set_min(-128);
	filler_param.set_max(127);
  ...
  virtual ~ConvolutionInt8LayerTest() {
    delete blob_bottom_;
    delete blob_bottom_2_;
    delete blob_top_;
    delete blob_top_2_;
  ...
  virtual Blob<int>* MakeReferenceTop(Blob<Dtype>* top) {
    this->ref_blob_top_.reset(new Blob<int>());
    this->ref_blob_top_->Reshape(top->shape());
    return this->ref_blob_top_.get();
  }
  ...
  Blob<Dtype>* const blob_top_;
  Blob<Dtype>* const blob_top_2_;
  shared_ptr<Blob<int> > ref_blob_top_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
};

TYPED_TEST_CASE(ConvolutionInt8LayerTest, TestDtypesAndDevices);

TYPED_TEST(ConvolutionInt8LayerTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
...
TYPED_TEST(ConvolutionInt8LayerTest, TestSimpleINT8Convolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
  layer_param.set_int8_inference(true);
  layer_param.set_activation_scale_factor(0.7);
  ...
  //*** Turn on filler of int8 weight in Layer.hpp
  convolution_param->mutable_weight_filler()->set_type("gaussian");
  convolution_param->mutable_weight_filler()->set_mean(0);
  convolution_param->mutable_weight_filler()->set_std(5);
  ...
  vector<Blob<int>*> top_int;
	...
	for (int id = 0; id < this->blob_top_vec_.size(); id++) {
  		Blob<int>* top_int_ = new Blob<int>(this->blob_top_->shape());
  		top_int.push_back(top_int_);
  }

  layer->Quantization_int8(this->blob_bottom_vec_, bottom_int8);
  layer->Forward_int8_cpu(bottom_int8, top_int);

  // Check against reference convolution.
  const int* top_data;
  const int* ref_top_data;
  //vector<shared_ptr<Blob<char> > > l_blobs_;
  //l_blobs_.push_back(layer->int8_blobs());
  caffe_conv(bottom_int8[0], convolution_param, layer->int8_blobs(),
      this->MakeReferenceTop(this->blob_top_));
  top_data = top_int[0]->cpu_data();
  ref_top_data = this->ref_blob_top_->cpu_data();
  for (int i = 0; i < this->blob_top_->count(); ++i) {
    EXPECT_NEAR(top_data[i], ref_top_data[i], 1e-4);
	//std::cout<<top_data[i]<<" "<<ref_top_data[i]<<"\n";
  }
  
  caffe_conv(bottom_int8[1], convolution_param, layer->int8_blobs(),
      this->MakeReferenceTop(this->blob_top_2_));
  top_data = top_int[1]->cpu_data();
  ref_top_data = this->ref_blob_top_->cpu_data();
  for (int i = 0; i < this->blob_top_->count(); ++i) {
    EXPECT_NEAR(top_data[i], ref_top_data[i], 1e-4);
	//std::cout<<top_data[i]<<" "<<ref_top_data[i]<<"\n";
...
TYPED_TEST_CASE(ConvolutionInt8E2ELayerTest, TestDtypesAndDevices);
float abss(float a){
	if(a>=0) return a;
	else return -a;
}
...
TYPED_TEST(ConvolutionInt8E2ELayerTest, TestEnd2EndSimpleINT8Convolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
  layer_param.set_int8_inference(true);
  float max = 0;
  for(int l=0;l<this->blob_bottom_vec_.size();l++){
	  const Dtype* blob_bottom_data = this->blob_bottom_vec_[l]->cpu_data();
	  for(int i=0;i<this->blob_bottom_->count();i++){
		  if(abss(blob_bottom_data[i])>max) max=abss(blob_bottom_data[i]);
	  }
  }
  layer_param.set_activation_scale_factor((float)127.0/(max*0.8));
  layer_param.clear_weight_scale_factor();
  layer_param.add_weight_scale_factor((float)127.0/(float)50.0);
  layer_param.add_weight_scale_factor((float)127.0/(float)50.0);
  layer_param.add_weight_scale_factor((float)127.0/(float)50.0);
  layer_param.add_weight_scale_factor((float)127.0/(float)50.0);
  ...
  convolution_param->mutable_weight_filler()->set_min(-128);
  convolution_param->mutable_weight_filler()->set_max(127);
  convolution_param->mutable_bias_filler()->set_type("constant");
  convolution_param->mutable_bias_filler()->set_value(0.1);
  shared_ptr<Layer<Dtype> > layer(
      new ConvolutionLayer<Dtype>(layer_param));
  ...
  shared_ptr<Blob<Dtype> > w_blob(new Blob<Dtype>(4, 3, 3, 3));
  const char* data_m = layer->int8_blobs()[0]->cpu_data();
  Dtype* data = w_blob->mutable_cpu_data();
  for(int oo=0;oo<4;oo++)
  for(int i=0;i<3;i++)
	  for(int j=0;j<3;j++)
		  for(int k=0;k<3;k++)
			  data[oo*3*9 + i*9 + j*3 + k] = (float)((int)data_m[oo*3*9 + i*9+j*3+k]) * (float)(50.0) / (float)(127.0);
  vector<shared_ptr<Blob<Dtype> > > w_blobs;
  w_blobs.push_back(w_blob);

  const Dtype* top_data;
  const Dtype* ref_top_data;
  caffe_conv_(this->blob_bottom_, convolution_param, w_blobs,
      this->MakeReferenceTop(this->blob_top_));
  top_data = this->blob_top_->cpu_data();
  ref_top_data = this->ref_blob_top_->cpu_data();
  for (int i = 0; i < this->blob_top_->count(); ++i) {
    //EXPECT_NEAR(top_data[i], ref_top_data[i], abs_(1*((float)ref_top_data[i])));
  }
  caffe_conv_(this->blob_bottom_2_, convolution_param, w_blobs,
      this->MakeReferenceTop(this->blob_top_2_));
  top_data = this->blob_top_2_->cpu_data();
  ref_top_data = this->ref_blob_top_->cpu_data();
  for (int i = 0; i < this->blob_top_->count(); ++i) {
    //EXPECT_NEAR(top_data[i], ref_top_data[i], abs_(1*((float)ref_top_data[i])));
	//printf("data1 %5.2f dat2 %5.2f different_rate = %f %\n", top_data[i], ref_top_data[i], ((top_data[i] - ref_top_data[i])/ref_top_data[i])*100);
  }
}

/*
TYPED_TEST(ConvolutionInt8LayerTest, TestDilatedConvolution) {
  typedef typename TypeParam::Dtype Dtype;
...
*/
/*
TYPED_TEST(ConvolutionInt8LayerTest, Test0DConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, TestSimple3DConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
...
TYPED_TEST(ConvolutionInt8LayerTest, TestDilated3DConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  vector<int> bottom_shape(5);
...
TYPED_TEST(ConvolutionInt8LayerTest, Test1x1Convolution) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, TestSimpleConvolutionGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, TestSobelConvolution) {
...
*/
/*
TYPED_TEST(ConvolutionInt8LayerTest, TestNDAgainst2D) {
  typedef typename TypeParam::Dtype Dtype;
...
TYPED_TEST(ConvolutionInt8LayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, TestDilatedGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, TestGradient3D) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, Test1x1Gradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ConvolutionInt8LayerTest, TestGradientGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
/*
template <typename Dtype>
class CuDNNConvolutionInt8LayerTest : public GPUDeviceTest<Dtype> {
 protected:
  CuDNNConvolutionInt8LayerTest()
  ...
  virtual ~CuDNNConvolutionInt8LayerTest() {
    delete blob_bottom_;
    delete blob_bottom_2_;
    delete blob_top_;
    delete blob_top_2_;
...
TYPED_TEST_CASE(CuDNNConvolutionInt8LayerTest, TestDtypes);

TYPED_TEST(CuDNNConvolutionInt8LayerTest, TestSetupCuDNN) {
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
...
*/
/*
TYPED_TEST(CuDNNConvolutionInt8LayerTest, TestSimpleConvolutionCuDNN) {
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
...
*/
/*
TYPED_TEST(CuDNNConvolutionInt8LayerTest, TestSimpleConvolutionGroupCuDNN) {
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  convolution_param->add_kernel_size(3);
...
*/
/*
TYPED_TEST(CuDNNConvolutionInt8LayerTest, TestSobelConvolutionCuDNN) {
  // Test separable convolution by computing the Sobel operator
  // as a single filter then comparing the result
  // as the convolution of two rectangular filters.
...
*/
/*
TYPED_TEST(CuDNNConvolutionInt8LayerTest, TestGradientCuDNN) {
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
...
*/
/*
TYPED_TEST(CuDNNConvolutionInt8LayerTest, TestGradientGroupCuDNN) {
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  convolution_param->add_kernel_size(3);
