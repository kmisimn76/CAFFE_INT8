...
// Blob<int> or Blob<unsigned int> or Blob<char>.
//INT8 Edited
...
void Blob<Dtype>::FromProto(const BlobProto& proto, bool reshape) {
  ...
  //INT8 Edited
  if (proto.has_int8_data()) {
	DLOG(INFO) << "INT8 Weight Load";
	const std::string datas = proto.int8_data();
	CHECK_EQ(count_, datas.length());
...
//INT8 Edited
template <>
void Blob<char>::ToProto(BlobProto* proto, bool write_diff) const {
	proto->clear_shape();
	for (int i = 0; i < shape_.size(); ++i) {
		proto->mutable_shape()->add_dim(shape_[i]);
	}
	proto->clear_data();
	const char* data_vec = cpu_data();
	proto->set_int8_data(data_vec);
}
