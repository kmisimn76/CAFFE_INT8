...
//	std::cout<<"extractMaxLayerOutput: ["<<iter<<"/"<<img_num<<"]\n";
	...
	while((dent = readdir(dp))) {
		...
		for (int i=0;i<quant_data.size();i++){
			...
			double min = quant_data[i].distribution.min_output_value;
			...
			quant_data[i].distribution.min_output_value = min; 
		}
...
//		std::cout<<layer<<"/"<<quant_data.size()<<" layer active quantize\n";
		...
		for (int M = m+quantize_level; M<INTERVAL_NUM; M++)
		{
			...
			for (int j=0;j<m;j++)
			{
				P.data[0] += quant_data[layer].distribution.data[j];
				sum_P += quant_data[layer].distribution.data[j];
			}
			for (int j=m;j<M;j++)
			{
				P.data[j-m] = quant_data[layer].distribution.data[j];
				sum_P += quant_data[layer].distribution.data[j];
			}
			for (int j=M;j<INTERVAL_NUM;j++)
			{
				P.data[M-1-m] += quant_data[layer].distribution.data[j];
				sum_P += quant_data[layer].distribution.data[j];
			}
