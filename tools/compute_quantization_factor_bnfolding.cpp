...
void computeBNFold(Net<float> &caffe_net, std::vector<LayerQuantData> &quant_data)
{
	int fold_count = 0;
	for (int i=0;i<quant_data.size();i++)
		if (quant_data[i].bn_fold) fold_count++;

	std::cout<<"BN Folding ["<<0<<"/"<<fold_count<<"]\n";
	for (int i=0;i<quant_data.size();i++)
	{
		if (!quant_data[i].bn_fold) continue;

		bool has_bias;
	   	if (caffe_net.layers()[quant_data[i].layer_n].get()->blobs().size() > 1)
			has_bias = true;
		else
			has_bias = false;
		...
		Blob<float>* bias;
		if (!has_bias) {
			//std::shared_ptr<Blob<float>> bias_ptr(new Blob<float>(weight->shape()[0], 1, 1, 1));
			//caffe_net.layers()[quant_data[i].layer_n].get()->blobs().push_back(std::shared_ptr<Blob<float>>(bias_ptr));
			caffe_net.layers()[quant_data[i].layer_n].get()->blobs().push_back(boost::shared_ptr<Blob<float>>(new Blob<float>(weight->shape()[0], 1, 1, 1)));
		}
		bias = caffe_net.layers()[quant_data[i].layer_n].get()->blobs()[1].get();
		if (bias->shape()[0] != weight->shape()[0]) {
			printf("not matched bias, weight shape\n");
			exit(1);
		}

		Blob<float>* b_mean = caffe_net.layers()[quant_data[i].layer_n+1].get()->blobs()[0].get();
		Blob<float>* b_var = caffe_net.layers()[quant_data[i].layer_n+1].get()->blobs()[1].get();
		Blob<float>* b_gamma = caffe_net.layers()[quant_data[i].layer_n+2].get()->blobs()[0].get();
		Blob<float>* b_beta = caffe_net.layers()[quant_data[i].layer_n+2].get()->blobs()[1].get();
		
		const float* mean_data = b_mean->cpu_data();
		const float* var_data = b_var->cpu_data();
		const float* gamma_data = b_gamma->cpu_data();
		const float* beta_data = b_beta->cpu_data();
...
void SaveQuantizedDeploy(std::string src_deploy_path, std::vector<LayerQuantData> &quant_data, std::string dest_deploy_path)
{
	...
	NetParameter net_param_dst;
	ReadProtoFromTextFile(src_deploy_path, &net_param);
	net_param_dst.CopyFrom(net_param);
	net_param_dst.clear_layer();

	int batch_norm_skip = 0;
	int layer_cnt = 0;
	...
	for (int i = 0; i < net_param.layer_size(); i++) {
		...
		if (batch_norm_skip==0 || batch_norm_skip==3) {
			net_param_dst.add_layer();
			LayerParameter layer_param_src = net_param.layer(i);
			LayerParameter* layer_param_dst = net_param_dst.mutable_layer(layer_cnt);
			layer_param_dst->CopyFrom(layer_param_src);
			layer_cnt++;
		}
		else if (batch_norm_skip==2) {
			const std::string batch_norm_name = net_param.layer(i).name();
			const std::string conv_name = net_param.layer(i-1).name();
			for (int k=0;k<net_param.layer_size();k++) {
				for (int l=0;l<net_param.layer(k).bottom_size();l++) {
					if (strcmp(net_param.layer(k).bottom(l).c_str(), batch_norm_name.c_str()) == 0) {
						std::string* str = net_param.mutable_layer(k)->mutable_bottom(l);
						(*str) = conv_name;
					}
				}
			}
			for (int k=0;k<net_param_dst.layer_size();k++) {
				for (int l=0;l<net_param_dst.layer(k).bottom_size();l++) {
					if (strcmp(net_param_dst.layer(k).bottom(l).c_str(), batch_norm_name.c_str()) == 0) {
						std::string* str = net_param_dst.mutable_layer(k)->mutable_bottom(l);
						(*str) = conv_name;
					}
				}
			}
		}
		if (batch_norm_skip>0) {
			batch_norm_skip--;
		}

	}
	WriteProtoToTextFile(net_param_dst, dest_deploy_path);
}

void SaveQuantizedModel(Net<float> &caffe_net, std::string src_model_path, std::vector<LayerQuantData> &quant_data, std::string dest_model_path)
{
	NetParameter net_param;
	NetParameter net_param_dst;
	...
	for (int i = 0; i < net_param.layer_size(); i++) {
		for (int j = 0; j < quant_data.size(); j++) {
			...
			Blob<float>* c_weight_blob = caffe_net.layers()[quant_data[j].layer_n].get()->blobs()[0].get();
			float* weight_data = c_weight_blob->mutable_cpu_data();
			if(layer_param->blobs_size() <= 1) {
				layer_param->add_blobs();
				BlobProto* bias_blob = layer_param->mutable_blobs(1);
				BlobShape* shape = bias_blob->mutable_shape();
				shape->clear_dim();
				shape->add_dim(blob->shape().dim(0));
				for(int kk = 0; kk<blob->shape().dim(0); kk++) {
					bias_blob->add_data(0.0);
				}
			}
			BlobProto* bias_blob = layer_param->mutable_blobs(1);
			Blob<float>* c_bias_blob = caffe_net.layers()[quant_data[j].layer_n].get()->blobs()[1].get();
			float* bias_data = c_bias_blob->mutable_cpu_data();

			//std::cout<<"int8 weights: "<<net_param.layer(i).name()<<": "<<blob->data_size()<<"\n";

			blob->clear_int8_data();
			bias_blob->clear_int_data();

			std::string int8_data;
			int8_data.reserve(blob->data_size());
			//int* int8_bias_data;
			//int8_bias_data.reserve(bias_blob->data_size());
			...
			for (int k = 0; k < blob->data_size(); k++) {
				float fp32_weight = weight_data[k];
				...
				/*if(quant_data[j].winograd){ // winograd
					...
					else if ( quantized_weight < -127 ) {
						quantized_weight = -127;
					}
				//}
				if(quant_data[j].weight_scale_factor[k/length] == 0.0) {
					quantized_weight = 0;
				}
			...
			for (int k = 0; k <  blob->shape().dim(0); k++) {
				...
				if ( quantized_bias >= 0.f) quantized_bias += 0.5;
				else quantized_bias -= 0.5;
...
/*					if ( quantized_bias > 127 ) {
						quantized_bias = 127;
					}
					else if ( quantized_bias < -128 ) {
						quantized_bias = -128;
					}*/
				//}
				int int8_bias = (int)quantized_bias;
				//printf("%f ", fp32_bias);
	...
	WriteProtoToBinaryFile(net_param_dst, dest_model_path);
}
...
int main(int argc, char** argv){
	...
	for (int i=0;i<net_layers.size();i++)
	{
		...
		if(strcmp(net_layers[i].get()->type(), "Convolution")==0 && (strstr(net_layers[i].get()->layer_param().name().c_str(), /*"landmark_pred"*/"pred")==NULL)){
			...
			data.layer_n = i;
			data.bn_fold = false;
			if(strcmp(net_layers[i+1].get()->type(), "BatchNorm")==0 && strcmp(net_layers[i+2].get()->type(), "Scale")==0) {
				data.bn_fold = true;
				printf("conv-bn fold\n");
			}
			else {
				printf("Exclusive: no batch norm conv: %s\n", net_layers[i].get()->layer_param().name().c_str());
			}
			quant_data.push_back(data);
		}
	}
	printf("qaunt_data size: %ld\n", quant_data.size());
	sleep(1);
	...
	// Weight Quantize
	computeBNFold(caffe_net, quant_data);
	...
	SaveQuantizedModel(caffe_net, argv[2], quant_data, argv[6]);
